...
function generateMetrics(baseTimestamp, serverIdx, hourOffset) {
  ...
  return {
    ...
    ping_ct: Math.max(10, baseline.ping * 1.2 + pingNoise),
    ping_cu: Math.max(10, baseline.ping + pingNoise),
    ping_cm: Math.max(10, baseline.ping * 1.1 + pingNoise),
    ping_bd: Math.max(10, baseline.ping * 1.5 + pingNoise),
